	...
	$(document).ready(function() {
		...
		$(".accepted_amount").on("change", function() {
			...
			if (loan_accept_amt > apply_amount) {
				showDialog("", "Total acccepted amount cannot be more than the Project Applied Amount");
				$(this).val("0.00");
				return;
			}
		...
		if (Number($("#loan_apply_amount").val()) > bid_amount) {
			if (numeral($("#partial_sub_allowed").val()).value() == 1) {
				if (numeral($("#min_for_partial_sub").val()).value() > bid_amount) {
					showDialog("", "Project has not been sufficiently subscribed. Cannot close bids");
					formValid = false;
				} else {
					formValid = true;
				}
