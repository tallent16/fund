 ...
 ------------------------------------------------------------------------*/
	...
	public function getAuditHeaderInfo($fromDate, $toDate, $filterModule, $filteraction) {
		...
		$auditSql	=	"	SELECT 	audit_key,
									user_id,	
									(select DEVFYN.users.username  from DEVFYN.users where fynAudit.audit_master.user_id =DEVFYN.users.user_id )	as username,								
									module_name,
									action_summary,
									action_detail,
		...
		$this->header_rs	=		$this->dbFetchWithParam($auditSql,$whereArray);	
