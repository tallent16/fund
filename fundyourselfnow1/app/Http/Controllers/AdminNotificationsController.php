...
class AdminNotificationsController extends MoneyMatchController {
	...
	public function followNow($loan,$status){
		 $user_type = Auth::user()->usertype;
		 $loan_id = base64_decode($loan);
		 $userID = $this->notificationsModel->getCurrentuserID();
		 $get = DB::table('loan_follows')
		 		->where('loan_id','=',$loan_id)
		 		->where('user_id','=',$userID)
		 		->get();
		 if($get){
		 	DB::table('loan_follows')
		 	->where('loan_id','=',$loan_id)
		 	->where('user_id','=',$userID)
		 	->update(array('status'=>$status));
		 } else{
		 	$data = array('loan_id' => $loan_id, 'user_id' => $userID, 'status' => '1');
		 	$res = DB::table('loan_follows')->insert($data);
		 }
		 if($user_type == '1'){
		 	return redirect(URL('creator/project_calander'));
		 }else if($user_type == '2'){
		 	return redirect(URL('backer/project_calander'));
		 }else{
		 	return redirect(URL('project_calander'));
		 }
	}
