...
class RolesServiceProvider extends ServiceProvider
{
    ...
    protected function registerBladeExtensions()
    {
		...
		$blade->extend(function ($view, $compiler) {
            $pattern = $compiler->createMatcher('checkRolePermission');

            return preg_replace($pattern, "<?php if (\\BorProfile::checkRolePermission$2) : ?>", $view);
        });

        $blade->extend(function ($view, $compiler) {
            $pattern = $compiler->createPlainMatcher('endcheckRolePermission');
