...
@section('section')  
	...
	@var	$toDate		=	$adminBorActRepMod->toDateFilterValue;
	@var	$borList	=	$adminBorActRepMod->allactiveBorrowerList;
...
<form 	class="form-horizontal" 
		id="excel_export" 
		method="post"
		action="{{url('admin/borrower-activity-report/download')}}">
		<input  type="hidden" 
				name="_token"
				id="hidden_token"
				value="{{ csrf_token() }}" />
...
@var	$showBorrowerName	=	false
@if(count($borFilter) > 1)
	@var	$showBorrowerName	=	true
@endif
@include('widgets.admin.borrower-activity', array(	"class"=>"",
													"borFilter"=>$borFilter,
													"borList"=>$borList,
													"showBorrowerName"=>$showBorrowerName))
